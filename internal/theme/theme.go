...
func Get(name string) Theme {
	...
	// List panes carry a themed bg; preview stays fg-only.
	//
	// The list panes (left/right) are rendered entirely by our own code
	// through a handful of well-known lipgloss styles — safe to paint bg
	// through to every cell. This gives the panes a clear "surface" feel
	// and makes the active/inactive affordance readable.
	//
	// The preview pane carries chroma/glamour output whose resets and
	// bg-blocks are too leaky to paint against a themed bg without a cat-
	// and-mouse game (tried it; lost). Instead it keeps its border fg and
	// lets the terminal bg show through, then gets extra border contrast
	// so the dashed `╌┊` frame reads as an intentional "floating" surface.
	//
	// Cursor / PaletteSelected / header / footer keep their explicit bg —
	// those are deliberately highlighted chrome.
	...
	// Preview: drop bg, bump border contrast to the theme accent.
	t.PreviewBorder = t.PreviewBorder.
		UnsetBackground().
